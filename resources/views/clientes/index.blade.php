...
<div class="row">
  <div class="col">
    <div class="card shadow">
      ...
      <div class="card-header border-0">
        <a href="{{route('cliente.create')}}" class="btn btn-primary me-md-1 btn-sm p-2"><i class="fas fa-plus"></i> Agregar</a>
        <a href="{{route('cliente.eliminados')}}" class="btn btn-warning btn-sm p-2">Eliminados</a>
      </div>
        ...
        <table class="table align-items-center table-flush">
          ...
          <tbody>
            @foreach ($tipos as $tipo)
              <tr>
                <td>
                  <span class="badge badge-dot mr-4">{{$tipo->id_cliente}}</span>
                </td>
                <td>
                ...
                <td>
                  ...
                  <form id="formEliminar{{$tipo->id_cliente}}" action="{{route('cliente.destroy',$tipo->id_cliente)}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <td>
                      <a href="{{route('cliente.edit', $tipo->id_cliente)}}">
                        <i class="fa-solid fa-pen-to-square" style="color: #e5e90c; font-size:20px;"></i>
                      </a>
                    </td>
                      <td>
                        <button type="button" onclick="mostrarModal('formEliminar{{$tipo->id_cliente}}');" style="border:none; background-color:#fff">

...
<div class="row">
  <div class="col">
    <div class="card shadow">
      ...
      <div class="card-header border-0">
        <a href="{{route('usuario.create')}}" class="btn btn-primary me-md-1 btn-sm p-2"><i class="fas fa-plus"></i> Agregar</a>
        <a href="{{route('usuario.eliminados')}}" class="btn btn-warning btn-sm p-2">Eliminados</a>
      </div>
        ...
        <table class="table align-items-center table-flush">
          ...
          <tbody>
            @foreach ($usuarios as $usuario)
              <tr>
                <td>
                  <span class="badge badge-dot mr-4">{{$usuario->usuario}}</span>
                </td>
                <td>
                  <span class="badge badge-dot mr-4">{{$usuario->password}}</span>
                </td>

                @foreach ($roles as $rol)
                    @if ($usuario->id_rol==$rol->id_rol)
                        <td>
                          <span class="badge badge-dot mr-4">{{$rol->nombre}}</span>
                        </td>
                    @endif
                @endforeach
                

                  <form id="formEliminar{{$usuario->usuario}}" action="{{route('usuario.destroy',$usuario->usuario)}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <td>
                      ...
                      <td>
                        <button type="button" onclick="mostrarModal('formEliminar{{$usuario->usuario}}');" style="border:none; background-color:#fff">

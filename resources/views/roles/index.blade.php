...
<div class="row">
  <div class="col">
    <div class="card shadow">
      ...
      <div class="card-header border-0">
        <a href="{{route('roles.create')}}" class="btn btn-primary me-md-1 btn-sm p-2"><i class="fas fa-plus"></i> Agregar</a>
        <a href="{{route('roles.eliminados')}}" class="btn btn-warning btn-sm p-2">Eliminados</a>
      </div>
        ...
        <table class="table align-items-center table-flush">
          ...
          <tbody>
            @foreach ($tipo as $rol)
              <tr>
                <td>
                  <span class="badge badge-dot mr-4">{{$rol->id_rol}}</span>
                </td>
                <td>
                  <span class="badge badge-dot mr-4">{{$rol->nombre}}</span>
                </td>
                <td>
                    <span class="badge badge-dot mr-4">{{$rol->descripcion}}</span>
                </td>
                  <form id="formEliminar{{$rol->id_rol}}" action="{{route('roles.destroy',$rol->id_rol)}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <td>
                      ...
                      <td>
                        <button type="button" onclick="mostrarModal('formEliminar{{$rol->id_rol}}');" style="border:none; background-color:#fff">
